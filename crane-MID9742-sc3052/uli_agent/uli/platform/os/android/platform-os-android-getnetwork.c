...
/*
  Convert ip integer ip address to string
*/

static UTV_RESULT s_IpIntToStr( UTV_UINT32 uiIp, UTV_INT8* strBuf )
{
  ...
  if ( NULL != strBuf )
  {
    sprintf( strBuf,
             "%u.%u.%u.%u",
             (UTV_UINT8)(uiIp & 0xFF),
             (UTV_UINT8)((uiIp >> 8) & 0xFF),
             (UTV_UINT8)((uiIp >> 16) & 0xFF),
             (UTV_UINT8)((uiIp >> 24) & 0xFF));
...
{ 
  ...
  if ( NULL != routes && NULL != pubBuf )
  {
    ...
    if ( 7 == iCnt )
    {
      ...
      s_IpIntToStr( uiGateway, sIpAddr );
      cJSON_AddStringToObject( routeAttrs, "Gateway", sIpAddr );
      s_IpIntToStr( uiMask, sIpAddr );
      cJSON_AddStringToObject( routeAttrs, "Mask", sIpAddr );
      ...
      s_IpIntToStr( uiDestination, sIpAddr );
...
/*
  ...
  Add the following information to the interfaces object:
    ...
    {"NAME_1" : {"IP Address" :         "192.169.10.100",
                 "Mask" :               "255.255.255.0",
    ...
    else
    {
      ...
      for ( i = 0; i < nInterfaces; i++ )
      {
        ...
        if ( ioctl( sck, SIOCGIFHWADDR, item ) >= 0 )
        {
          ...
          cJSON_AddStringToObject( interfaceAttrs, "MAC Address", sMacAddr );
        }
        ...
        if ( NULL != routes )
        {
          ...
          cJSON_AddStringToObject(
            interfaceAttrs,
            "Gateway",
            (routeGateway != NULL ? routeGateway->valuestring : "") );
        }
...
/* UtvGetNetworkInfo
   ...
   Add the following information to the response data:
    ...
    {"Interfaces" : { SEE s_GetInterfacesData },
     "Routes" :     { SEE s_GetRoutesInfo }}
*/
