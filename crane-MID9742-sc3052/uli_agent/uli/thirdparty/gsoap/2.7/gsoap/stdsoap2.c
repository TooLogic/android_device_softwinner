...
#ifdef __cplusplus
SOAP_SOURCE_STAMP("@(#) stdsoap2.cpp ver 2.7.15 2009-11-07 00:00:00 GMT")
extern "C" {
#else
SOAP_SOURCE_STAMP("@(#) stdsoap2.c ver 2.7.15 2009-11-07 00:00:00 GMT")
#endif
...
#ifdef GSOAP_SSL_ALLOW_HOST_ENDING_MATCH
/* UpdateLogic: Enabling cert subject/host match by ending only */
static int hostendingmatch(const char *subject, const char *host);
#endif
...
/* UpdateLogic: Enabling cert subject/host match by ending only */
static int
hostendingmatch(const char *subject, const char *host)
{
  int match = 0;

  if ((subject) && (host))
  { /* subject must contain at least two components */
    size_t subjectlen = strlen(subject);

    if ((subjectlen > 2) && strchr(subject, '.')
        && (subject[0] != '.') && (subject[subjectlen - 1] != '.'))
    { /* host must be at least as long as subject */
      size_t hostlen = strlen(host);

      if (hostlen >= subjectlen)
      { /* host must end with subject */
        const char* hostend = &(host[hostlen - subjectlen]);

        if (!strcasecmp(hostend, subject))
        { match = 1;
        }
      }
    }
...
#ifdef GSOAP_SSL_ALLOW_HOST_ENDING_MATCH
/* UpdateLogic: Enabling cert subject/host match by ending only */
                    if (nval && !strcmp(nval->name, "DNS") && hostendingmatch(nval->value, host))
                    { ok = 1;
                      break;
                    }
#endif
...
#ifdef GSOAP_SSL_ALLOW_HOST_ENDING_MATCH
/* UpdateLogic: Enabling cert subject/host match by ending only */
              else if (hostendingmatch((const char*)name, host))
                ok = 1;
#endif
              else
...
#ifdef GSOAP_SSL_ALLOW_HOST_ENDING_MATCH
/* UpdateLogic: Enabling cert subject/host match by ending only */
                  else if (hostendingmatch((const char*)tmp, host))
                    ok = 1;
#endif
                  OPENSSL_free(tmp);
                }
...
{
/* UpdateLogic: Preventing "unreferenced local variable" compiler warning. */
#ifndef WITH_LEAN
  register const char *s;
#endif
